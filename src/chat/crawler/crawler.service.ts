...
export class CrawlerService {
    ...
    public getCrawler = async (message: string): Promise<CrawDataGoogle[]> => {
        let dataCrawler: CrawDataGoogle[] = [];
        try {
            ...
            // Lyrics
            let lyric = $(data).find('div.PZPZlf.zloOqf').first();
            if (lyric.length > 0) {
                let lyricData = [];
                lyric.each((i, data) => {
                    let lyrics = $(data).find('span');
                    lyrics.each((i, data) => {
                ...
                if (lyricData.length > 0) {
                    for (let i = 0; i < lyricData.length; i += 10) {
                        let lyric = lyricData.slice(i, i + 10).join('\n');
    ...
    public getLuckyNumber = (message: string): CrawDataLucky => {
        ...
        if (min && max && get) {
            ...
            for (let i = 0; i < get; i++) {
                let lucky = this.getRanDomBetween(min, max);
            ...
            // random tu 1 den 100
            let lucky = this.getRanDomBetween(1, 100);
            dataLucky.data = [String(lucky)];
            dataLucky.message =
